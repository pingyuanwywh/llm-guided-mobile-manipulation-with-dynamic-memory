#!/bin/bash
# Render the 2026-08-15 drone5 dynamic RVIZ/godview to mp4.
set -euo pipefail

RUN="$HOME/jetrover_demo/raw/20260815_2120_drone5_2station-5can-SUCCESS-5of5"
EDITED="$HOME/jetrover_demo/edited/20260815_2120_drone5_2station-5can-SUCCESS-5of5/intermediate"
BAG="$RUN/0815_drone5.bag"
...
case "$MODE" in
preview)
  OUT="${OUT:-/tmp/drone5_0815_dynamic_rviz_preview.mp4}"
  DUR="${DUR:-35}"
  ;;
full)
  OUT="${OUT:-$EDITED/20260815_2120_drone5_dynamic_rviz_v2_corrected_full.mp4}"
  DUR="${DUR:-402}"
  ;;
*)
...
cleanup() {
  ...
  $HOME/replay_0815_drone5.sh stop >/dev/null 2>&1
  if [ -n "${XVFB_PID:-}" ]; then
...
$HOME/replay_0815_drone5.sh stop >/dev/null 2>&1 || true
pkill -f "[X]vfb $DISPLAY_ID" >/dev/null 2>&1 || true

Xvfb "$DISPLAY_ID" -screen 0 "$SCREEN" -ac >/tmp/drone5_0815_xvfb.log 2>&1 &
XVFB_PID=$!
sleep 2

$HOME/replay_0815_drone5.sh start >/tmp/drone5_0815_render_replay.log 2>&1

rviz -d "$RVIZ_CFG" >/tmp/drone5_0815_rviz.log 2>&1 &
sleep 10
...
sleep 2
rosbag play --clock --quiet "$BAG" >/tmp/drone5_0815_rosbag_play.log 2>&1 &
ROSBAG_PID=$!
