...
# 08-08: 支持换配置(离线重渲染用带 marker 的那份), 默认仍是原来那个
RVIZ_CFG="${RVIZ_CFG:-$HOME/.rviz/jetrover_demo.rviz}"
MY_IP="${MY_IP:?未设置 MY_IP：请配置 ~/.jetrover_env}"

if [ "${1:-start}" = "stop" ]; then
  pkill -f "[r]viz -d ${RVIZ_CFG}"; pkill -f "[r]viz -d $HOME/.rviz/jetrover_demo"
  pkill -f "[X]vfb $VD"
...
echo "+ DISPLAY=$VD rviz -d $RVIZ_CFG"
